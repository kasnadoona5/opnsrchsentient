...
This repository provides a step-by-step guide to deploying the OpenDeepSearch library as a robust, persistent API endpoint using FastAPI. This setup is ideal for integrating OpenDeepSearch(https://github.com/sentient-agi/OpenDeepSearch) into other applications like n8n, Zapier, or any custom workflow that needs to call a reliable, long-running search agent.
...
- An API key from a language model provider (this guide uses [OpenRouter.ai](https://openrouter.ai))
...
```bash
curl -X POST "http://your-server-ip:8000/search" \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_SUPER_SECRET_RANDOM_STRING_HERE" \
  -H "serper-api-key: YOUR_SERPER_API_KEY" \
  -H "openrouter-api-key: YOUR_OPENROUTER_API_KEY" \
  -d '{"query": "What are the latest advancements in AI agents?"}'
```
...
1. **Port already in use**: If port 8000 is busy, change it in the uvicorn command: `--port 8001`
2. **Module not found**: Make sure you're in the correct directory and virtual environment is activated
3. **API keys not working**: Verify your API keys are valid and have sufficient credits/permissions

### Viewing Logs

To see real-time logs, re-attach to your screen session:
...
Feel free to submit issues and pull requests to improve this setup guide or the API implementation.

## License

This project is open source. Please check the OpenDeepSearch library's license for usage terms.
